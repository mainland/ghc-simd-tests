...
class (MultiPrim a, V.Vector v a, PackedMVector (V.Mutable v) a) => PackedVector v a where
...
instance PackedVector U.Vector Int where
    unsafeIndexAsMulti (U.V_Int (P.Vector i _ arr)) j =
        indexByteArrayAsMulti arr (i+j)

instance PackedMVector U.MVector Int where
    unsafeReadAsMulti (U.MV_Int (P.MVector i _ arr)) j =
        readByteArrayAsMulti arr (i+j)

    unsafeWriteAsMulti (U.MV_Int (P.MVector i _ arr)) j x =
        writeByteArrayAsMulti arr (i+j) x
#endif
