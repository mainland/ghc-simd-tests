...
kde_ n0 min max xs
  ...
  where
    mesh = V.generate ni $ \z -> min + (d * fromIntegral z)
        where d = r / (n-1)
    density = V.map (/(2 * r)) . idct $ V.mzipWith f mf a (V.enumFromTo 0 (n-1))
      where
        f :: Double -> Double -> Double
        f b z = b * exp (sqr z * sqr pi * t_star * (-0.5))

        mf :: Multi Double -> Multi Double -> Multi Double
        mf b z = b * exp (sqr z * multireplicate (sqr pi * t_star * (-0.5)))
    ...
    !t_star = fromRoot (0.28 * len ** (-0.4)) . ridders 1e-14 (0,0.1) $ \x ->
      ...
      where
        f :: Double -> Double -> Double
        f q t = 2 * pi ** (q*2) * V.vsum (V.mzipWith g mg iv a2v)
          where g i a2 = i ** q * a2 * exp ((-i) * sqr pi * t)
                mg i a2 = i ** multireplicate q * a2 * exp ((-i) * multireplicate (sqr pi * t))

                a2v, iv :: V.Vector Double
                a2v = V.mmap (sqr . (*0.5)) (sqr . (*0.5)) a
                iv  = V.mmap sqr sqr $ V.enumFromTo 0 (n-1)
        go s !h | s == 1    = h
                | otherwise = go (s-1) (f s time)
          where time  = (2 * const * k0 / len / h) ** (2 / (3 + 2 * s))
                const = (1 + 0.5 ** (s+0.5)) / 3
                k0    = V.vproduct (V.enumFromThenTo 1 3 (2*s-1)) / m_sqrt_2_pi
    sqr x = x * x
