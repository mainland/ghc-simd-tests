...
import "dph-lifted-vseg" Data.Array.Parallel.Prim                 ()
import Data.Array.Parallel.Prelude.Base         (Bool, Int, Double, Eq, Ord, Num)
import Data.Array.Parallel.PArr
import "dph-lifted-vseg" Data.Array.Parallel.PArray
import "dph-lifted-vseg" Data.Array.Parallel.Lifted                       ((:->)(..))
import qualified "dph-lifted-vseg" Data.Array.Parallel.Lifted             as L
import qualified Data.Array.Parallel.PArray.Scalar      as SC
import qualified Prelude as P
import "vector" Data.Primitive.Multi
import "vector" Data.Primitive.Multi.DoubleX2
...
sumPP          = L.closure1' sum1 (SC.folds (+) 0)
  where
    sum1 :: PArray Double -> Double
    sum1 =
        SC.mfold plus1 plusm (0, 0) red (P.+)
      where
        plusm (x, mx) my = (x, mx P.+ my)
        plus1 (x, mx) y  = (x P.+ y, mx)
        red (s, ms)      = multifold (P.+) s ms
